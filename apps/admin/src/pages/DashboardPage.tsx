...
import { resetUnityScene } from "../api/engine";
...
import type { EngineTransportStatus } from "../types/realtime";
...
export function DashboardPage() {
  ...
  const [engineTransports, setEngineTransports] = useState<{
  ...
  const [resettingUnityScene, setResettingUnityScene] = useState(false);
  const [unityActionMessage, setUnityActionMessage] = useState<string | null>(null);
  const [unityActionError, setUnityActionError] = useState<string | null>(null);
  ...
  useEffect(() => {
    const unsubscribe = subscribeToTwitchRealtime({
      ...
      onSnapshot: (snapshot) => {
        ...
        setEngineTransports(snapshot.engineTransports ?? null);
      ...
      onEngineStatus: (value) => {
        setEngineTransports(value);
      },
  ...
  async function handleResetUnityScene() {
    setResettingUnityScene(true);
    setUnityActionMessage(null);
    setUnityActionError(null);

    try {
      const result = await resetUnityScene();
      setUnityActionMessage(`Unity scene reset sent (${result.deliveredCount} clients)`);
    } catch (error) {
      setUnityActionError(
        error instanceof Error ? error.message : "Не вдалося надіслати reset_scene в Unity",
      );
    } finally {
      setResettingUnityScene(false);
    }
  }
  ...
  return (
    <div className="page">
      <div className="card">
        <div className="card__header">
          ...
          <div className="actions">
            <button
              ...
              type="button"
              onClick={() => void handleResetUnityScene()}
              disabled={resettingUnityScene || !engineTransports?.unity.clientsCount}
            >
              {resettingUnityScene ? "Reset..." : "Reset Unity scene"}
            </button>
        ...
        {unityActionError ? (
          <div className="state-block state-block--error">{unityActionError}</div>
        ) : null}

        {unityActionMessage ? (
          <div className="state-block">{unityActionMessage}</div>
        ) : null}
        ...
        <div className="status-grid">
          ...
          <div className="status-item">
            <span className="status-item__label">UE WS clients</span>
            <strong>{engineTransports ? engineTransports.unreal.clientsCount : "—"}</strong>
          </div>

          <div className="status-item">
            <span className="status-item__label">Unity WS clients</span>
            <strong>{engineTransports ? engineTransports.unity.clientsCount : "—"}</strong>
          </div>
        </div>
      </div>
                ...
                <article className="event-card" key={entry.id}>
                  <div className="event-card__top">
                    <div className="event-card__meta">
                      ...
                      {entry.actionDispatchStatus ? (
                        <span className="badge badge--muted">action: {entry.actionDispatchStatus}</span>
                      ) : null}
                    </div>
            ...
            <div className="events-list dashboard-scroll-area">
              {rewardHistory.map((entry) => (
                <article className="event-card" key={entry.id}>
                  ...
                  {entry.unrealEventName ? (
                    <div className="event-card__meta">
                      <span className="badge badge--muted">UE: {entry.unrealEventName}</span>
                      {entry.unityEventName ? (
                        <span className="badge badge--muted">
                          Unity: {entry.unityEventName}
                        </span>
                      ) : null}
                      {entry.targetTransports?.length ? (
                        <span className="badge badge--muted">
                          targets: {entry.targetTransports.join(", ")}
                        </span>
                      ) : null}
                    </div>
                  ) : null}
                  ...
                  {entry.payloadSummary ? (
                    <div className="event-card__message">Payload: {entry.payloadSummary}</div>
                  ) : null}

                  {entry.dispatchedPayload ? (
                    <details className="event-card__details">
                      <summary className="event-card__details-summary">Показати dispatch payload</summary>
                      <pre className="event-card__data">
                        {JSON.stringify(entry.dispatchedPayload, null, 2)}
                      </pre>
                    </details>
                  ) : null}
                </article>
